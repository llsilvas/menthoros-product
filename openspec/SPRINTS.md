...
| 9i | `coach-add-workout-to-plan` *(S, visível, backend+front)* | ~25 | **Fecha o ciclo de adição.** Coach inclui treinos não previstos pela IA durante revisão (`POST /coach/planos/{id}/treinos`); dialog com etapas dinâmicas; migration V41 (`adicionado_pelo_coach`); aviso double-day; guardrail 14 treinos/semana; chip "Adicionado pelo coach". | `coach-edit-planned-workout` ✅ |
