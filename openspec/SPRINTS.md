...
| 9.5 | `wire-athlete-shell-to-endpoints` *(S · Fast, **frontend-only**)* | 0/~13 | **Fecha o loop coach→atleta antes da demo (Home + Plano + Chat).** Reescopada 2026-07-03 após product-review (Refine) + decisão do founder: split. Esta change fica com o núcleo forte — Home e Plano do atleta consumindo dado real (`GET /me/home`, `/me/readiness`, `/planos/{id}` — **todos já existem em develop, zero endpoint novo**) + chat vira placeholder honesto. `readiness.factors` granular removido (sem fonte, D0.3); "TSS meta" → volume planejado/realizado (D0.5). Demo abre o app do atleta (confirmado pelo founder) → bloqueador real do próximo pitch. Ainda não iniciada. | `add-athlete-progress-endpoints` ✅, `coach-plan-review-workflow` ✅ |
| 9.6 | `wire-athlete-progress-to-endpoints` *(M · Full, backend+front)* | 0/~18 | **Progresso do atleta (engajamento).** Separada da 9.5 (justificativa distinta — atleta olhando o próprio histórico, não fechamento do loop; e onde vivem os endpoints novos). 4 endpoints `/me/*` (`metricas/historico`, `metricas/zonas`, `recordes`, `aderencia` — todos espelham métodos de serviço já existentes, D0.1) + wiring de `AthleteProgressPage`; KPI de volume derivado de `/me/treinos` já existente (D0.2). Independente da 9.5 (arquivos frontend disjuntos), sequenciada depois (9.5 é demo-crítica). | `add-athlete-progress-endpoints` ✅, `manual-training-entry-lightweight` ✅ |
| 9.7 | `add-athlete-engagement-signals` *(XS, backend+front)* | 0/~13 | **Streak de consistência + próxima prova/meta (engajamento do atleta).** Recuperada 2026-07-03: ideia surgida numa sessão CPO anterior, encaixada inicialmente em `wire-athlete-shell-to-endpoints`, mas ficou de fora sem rejeição explícita no split/refino daquela change — reencaixada aqui como change própria e pequena. Streak calculado client-side sobre `/me/treinos` (já existe); próxima prova via 1 endpoint novo `/me/provas` (espelha `ProvaService.listarProvas`, já usado pelo coach). Fatia athlete-facing de baixo custo do discovery de retenção (`prd/prd-retention-loop-90d.md`) — não antecipa nem substitui o Retention Radar coach-facing (Sprint 26+, founder-gated). Sequenciada após 9.6 (mesmos arquivos de frontend). | `manual-training-entry-lightweight` ✅ |
