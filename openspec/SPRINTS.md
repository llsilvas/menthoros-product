...
**Última atualização:** 2026-06-26 (tríade do dashboard do coach concluída: `fix-coach-inbox-metrics`, `consolidate-coach-inbox-tabs`, `coach-training-strain` e `coach-race-form-prediction` arquivadas; `introduce-coach-assistant-core-features` / `coach-assistant-triage-ui` concluídos e mergeados — endpoint agregado do dashboard, triagem UI do coach (inbox decomposto, aderência e nível no roster), aderenciaPercentual no roster; próxima: `add-llm-tool-use` sprint 10–11)
