...
**Última atualização:** 2026-06-30 (**Go-live waitlist [1/4]:** `add-privacy-policy` mergeada (Fast track, sem change formal) — Política de Privacidade LGPD em `/privacidade` (10 seções: controlador, dados, base legal Art. 7º I, direitos Art. 18, etc.), fechando a task 4.3 deferida da `add-waitlist`; PR front #25. Pendente p/ go-live: e-mail de contato real, hardening XFF+CORS prod, logo/foto-bio reais. Antes: `redesign-landing-premium` mergeada — landing premium token-driven + captura inline na waitlist + vídeo de showcase no hero + fix da rota `/` (home legada → `/inicio`) + link "Entrar"; PR front #22; arquivada em `archive/2026-06/2026-06-30-redesign-landing-premium`. **Supera `marketing-landing-page`** (arquivada junto). Antes: `add-waitlist` mergeada — waitlist pública pré-lançamento: `POST /api/v1/waitlist` + tela `/waitlist` + CTAs da landing repontados; PRs backend #14 + front #21; arquivada em `archive/2026-06/2026-06-29-add-waitlist`. Antes: 2026-06-27 — `standardize-coach-dialogs` mergeada — fecha o arco dos dialogs do coach: shell único `CoachDialog` + helpers de botão + fonte única de cor de treino, todos movidos para `src/shared/`, PR #19; `align-coach-dialogs-base-plano` arquivada — fundação `CoachDialog`; migração `coach/atletas` concluída — `coach-roster-operational-actions` + `coach-roster-athlete-crud` arquivadas; tríade do dashboard do coach concluída: `fix-coach-inbox-metrics`, `consolidate-coach-inbox-tabs`, `coach-training-strain` e `coach-race-form-prediction` arquivadas; `introduce-coach-assistant-core-features` / `coach-assistant-triage-ui` concluídos e mergeados — endpoint agregado do dashboard, triagem UI do coach (inbox decomposto, aderência e nível no roster), aderenciaPercentual no roster; próxima: `add-llm-tool-use` sprint 10–11)
