...
| 9.5 | `wire-athlete-shell-to-endpoints` *(M, visível, backend+front)* | ~16 | **Fecha o loop coach→atleta antes da demo.** Shell do atleta inteiro roda em mock (`AthleteHomePage`, `AthletePlanPage`, `AthleteProgressPage`, `AthleteCoachPage`); endpoints de dado já existem (`add-athlete-progress-endpoints` ✅) e plano aprovado já é filtrado por role (`PlanoTreinoController`). 3 endpoints `/me/*` novos (backend) + wiring frontend (mesmo padrão de `wire-coach-shell-to-dashboards`). **Priorizado sobre `add-llm-tool-use`/RAG** (decisão do founder, 2026-07-03): North Star critério 4 + momento de valor do coach (passo 4/5) ficam incompletos sem isso, e o custo é baixo (dado já existe). Chat coach↔atleta vira placeholder honesto (depende de Sprint 25). | `add-athlete-progress-endpoints` ✅, `coach-plan-review-workflow` ✅, `manual-training-entry-lightweight` ✅ |
