...
| 6-DX | 🔧 `fix-openapi-client-generation` *(tech-debt/DX)* | L | **Restaura `generate:api` determinístico.** **Fase A (backend `@Tag` ASCII) ✅ em `develop`** (c8d1b95); **Fase B (regen + migração de tipos no front) pendente** — agora desbloqueada (6b estabilizada). Change segue **ativa** até a Fase B. | — (Fase B usa 6b concluída) |
