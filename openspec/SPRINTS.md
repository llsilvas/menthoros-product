...
**Última atualização:** 2026-06-21 (hotfix 9g.1 especificado: `fix-intervalado-distance-calculation` — distância incorreta em treinos intervalados; próxima: `coach-edit-planned-workout` sprint 9g)
...
| 9g.1 | `fix-intervalado-distance-calculation` *(XS, backend)* | ~10 | **Hotfix: distância errada em treinos intervalados.** LLM usa pace de tiro (Z5) para estimar `distanciaKm` de etapas de aquecimento/desaquecimento/recuperação → 10min de aquecimento vira 2.5km em vez de ~1.4km → total 10km em vez de ~6km. Fix: pós-processamento determinístico em `IaServiceImpl` que deriva `distanciaKm` das etapas temporais via `duracaoMin ÷ paceZona` antes de `normalizarTreinoIntervalado()`. | — |
