...
**Última atualização:** 2026-06-22 (`coach-edit-planned-workout` sprint 9g concluído e mergeado — edição granular de treinos planejados pelo coach entregue; próxima: `infer-thresholds-from-recent-workouts` sprint 9h)
...
| 9g | `coach-edit-planned-workout` *(S, visível, backend+front)* | ~22 | **Fecha o ciclo de revisão.** Coach edita treinos individuais durante revisão do plano (`AGUARDANDO_REVISAO`): tipo, distância, duração, zona, RPE, TSS (recalculado automaticamente ou override manual). `PATCH /coach/planos/{id}/treinos/{id}`; migration V39 (`editado_pelo_coach`); chip "Editado manualmente" na UI; `editadoPeloCoach = true` alimentará `rag-coach-methodology-personalization` (Sprint 17). | `coach-plan-review-workflow` ✅ | **✅ CONCLUÍDO 2026-06-22** |
