...
| 6-DX | 🔧 `fix-openapi-client-generation` *(tech-debt/DX)* | L | **Restaura `generate:api` determinístico.** **Fase A (`@Tag` ASCII) ✅** + **Fase A2 (schemas `array` de lista: listarAtletas/listarProvas) ✅** em `develop` (24d606a); **Fase B (regen + migração no front) pendente** — desbloqueada após reiniciar o backend. Change segue **ativa** até a Fase B. | — |
