...
| 6b | `wire-coach-shell-to-dashboards` *(visível, front)* | M | **Liga as 3 telas do coach aos endpoints reais** (hoje em mock): cliente OpenAPI + hooks + reconciliação DTO↔view-model (mapear/derivar/adiar). Fecha o valor visível da sprint 6 no front. | `add-coach-shell-dashboards` ✅ |
| 6-DX | 🔧 `fix-openapi-client-generation` *(tech-debt/DX)* | L | **Restaura `generate:api` determinístico.** Fase A (backend `@Tag` ASCII) mergeável já e pode preceder a 6b; Fase B (regen + migração de tipos no front) após a 6b estabilizar. Resolve a divergência cliente-curado↔doc descoberta no init da 6b. | — (Fase B usa 6b concluída) |
...
> `harden-plan-generation-resilience` foi **folded em `migrate-plan-prompt-to-skills`** (Sprint 6–8, seção 9 do tasks.md): a resiliência estrutural é construída sobre a base nova da migração em vez de remendar o `IaServiceImpl` que será desmontado.
