...
- [ ] 1.1 Migrar para `CoachDialog` (ou aplicar `PaperProps` equivalentes): remover `Paper` branco (`#ffffff`)
...
- [ ] 2.1 Boxes de seção do form (gradientes claros) → `elevation.card` + `border content.cardBorder`.
- [ ] 2.2 Chip "Edição/Cadastro" (`rgba(255,255,255,0.12)`/`#e8eaed`) → tokens.
- [ ] 2.3 Tipografia: `#1a2535`/`#6b7a8d`/demais → `surface[50]/[200]/[400]`; manter Syne nos títulos.
  - verify: contraste legível (labels/inputs/placeholder) no fundo escuro.

## Seção 3 — Ações

- [ ] 3.1 `DialogActions` (`#f8fafc`/`#e2e8f0`) → rodapé `CoachDialog`; botões padrão (primário
...
- [ ] 4.3 Validar criar+editar pelo roster **e** pelo `/atletas` legado (mesmo componente).
- [ ] 4.4 Marcar itens `[x]`.
