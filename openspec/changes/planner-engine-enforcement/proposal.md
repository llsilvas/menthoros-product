...
- **CA5/CA9 — Formatter so renderiza quando `enabled=true`:** `PeriodizacaoPromptFormatter` renderiza exclusivamente a saida do planner com `enabled=true`; com `enabled=false` preserva o calculo legado byte-a-byte. Some so a divergencia dual-calc do formatter (caminho enabled); a metrica de divergencia do **shadow** (parte 1) e **preservada** para o gate de rollout, aposentada so apos a promocao geral.
...
- **CA11 — Gate de rollout em DUAS PORTAS (design Decisao 5):** **Porta 1 (entrada no piloto, coorte restrita)** exige apenas a divergencia de fase do shadow (`planner.phase.divergence.count / planner.generated.count`, metrica da parte 1, disponivel com `enabled=false`) **<= 2%** em janela **>= 2 semanas** com **>= 30 planos** + defaults seguros. **Porta 2 (promocao geral)** exige as metricas coletadas no piloto, por coorte/fase, com limiares fixados: retry < 15%, `FAILED` < 5%, fallback < 5%, e `SugestaoCoach` MODIFIED/REJECTED nao pior que o baseline. Qualquer criterio da porta acima do limiar / metrica indisponivel / amostra insuficiente = **fail-closed**, nao avanca. Medicao e veredito na task 8.4 antes de cada porta.
