...
5. **Belt-and-suspenders (tipo sempre presente no realizado)** — se `tipoTreino` ∈ {INTERVALADO, FARTLEK, TIRO, SUBIDA}, retorna `null` **mesmo que o CV passe** (protege contra treino real mal-segmentado, ex.: 2 laps grosseiros de um HIIT; `SUBIDA` = "Tiros de Subida", non-steady por definição). `PROVA` e `TEMPO_RUN` ficam **de fora deliberadamente** — esforços em ritmo sustentado onde o decoupling é justamente útil. `tipoTreino` é `nullable=false` na entidade (`TreinoBase`), então este predicado está sempre ativo; o guarda `tipoTreino == null` existe só por robustez defensiva (cai no CV). Lista revisável pós-release (heurística v1).
