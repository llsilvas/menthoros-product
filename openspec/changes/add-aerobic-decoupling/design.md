...
4. **Steady por variabilidade** (robusto, independe da classificação manual):
   ...
   - Intervalado/fartlek têm picos e vales → CV alto → reprovado. Thresholds (`CV_FC_MAX`, `CV_VEL_MAX`) calibráveis, ancorados em "steady = baixa variação".
5. **Belt-and-suspenders** — se `TipoTreino` for explicitamente intervalado/fartlek/tiro, retorna `null` **mesmo que o CV passe** (protege contra treino real mal-segmentado, ex.: 2 laps grosseiros de um HIIT).
6. **Sanidade** — qualquer FC/velocidade/duração `= 0`, nula ou implausível → `null`.

`null` é semântico ("não aplicável"), distinto de zero. O front trata `null` como estado dedicado (AC4). Os thresholds vivem como constantes nomeadas no service (não mágicos inline), facilitando calibração.
...
- **Componente:** um `DecouplingBadge` (pequeno, reutilizável) — recebe `value: number | null`:
  ...
  - número → `{value}%` colorido por faixa **+ a linha de interpretação** (número passivo é fácil de ignorar; a leitura em palavras aumenta a chance de o coach agir):
    - `< 5%` → `semantic.success[500]` · "boa durabilidade aeróbica"
    - `5–10%` → `semantic.warning[500]` · "durabilidade moderada"
    - `> 10%` → `semantic.danger[500]` · "desacopla sob fadiga"
