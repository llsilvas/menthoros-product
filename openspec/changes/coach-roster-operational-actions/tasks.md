...
- [x] 1.1 Estado de ação: `action: 'plano' | 'projecao' | null`, `actionTarget: {atletaId, nome} | null`.
- [x] 1.2 Coluna final "Ações" (`sortable:false`, largura fixa) com `IconButton(MoreVert)` que abre um
...
- [x] 2.1 Renderizar `PlanosDialog` controlado por `action === 'plano'` (passa `atletaId`/`atletaNome` do alvo).
- [x] 2.2 Ao gerar/excluir plano com sucesso, chamar `fetchRoster()`.
  - verify: abrir Plano lista planos do atleta; gerar cria; excluir remove; roster reflete.

## Seção 3 — Sincronizar Strava

- [x] 3.1 Acionar `SyncStravaButton`/`useStravaSync(atletaId)` pelo item de menu (`connected={false}`,
...
- [x] 4.1 Renderizar `GerarProjecaoDialog` controlado por `action === 'projecao'` (atletaId/nome do alvo);
...
- [x] 5.1 Teste de comportamento (Vitest + Testing Library): o menu abre e cada item dispara o dialog/ação
  correto para o atleta da linha (mockar serviços/hooks).
- [x] 5.2 Regressão: grade de leitura permanece funcional.
  - verify: `npm run test:run` verde.

## Seção 6 — Validação final

- [x] 6.1 `npm run lint && npm run build && npm run test:run`.
- [x] 6.2 `/qa` (frontend-reviewer + clean-code-reviewer) sem achado Critical.
- [x] 6.3 Marcar itens `[x]` neste `tasks.md`.

## Seção 7 — Entrega

- [x] 7.1 Commits por seção lógica (Conventional Commits PT-BR).
