...
- [x] 1.3 Teste primeiro (CA4): cenário em que a soma já está dentro de 0,05 km do alvo — nenhuma
      etapa é alterada.
- [x] 1.4 CA5 (gap positivo) já coberto pelos testes existentes de crescimento
      (`IaServiceImplFartlekExpansaoTest`/estrutura de `distribuirDeltaPorTipo`); o caminho de
      `gap > 0` não foi tocado pela mudança — só o de `gap < 0` passou a excluir `INTERVALADO`.
...
- **Validação:** `./mvnw clean test` — `IaServiceImplNormalizarIntervaladoTest` (3/3) e suíte
  `IaServiceImpl*Test` (63/63) verdes.
