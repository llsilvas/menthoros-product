...
- **Propagação de contexto no loop de tools (decisão pós pre-mortem cross-model, 2026-09-18).** `ThreadLocal`/`@RequestScope` não são seguros no loop de tools do Spring AI — o loop pode rodar em thread diferente da request, ou continuar depois que o streaming iniciou, vazando ou perdendo o contexto. **Decisão:** propagar `ToolExecutionContext` via `ToolContext` nativo do Spring AI (`ChatClient.prompt().toolContext(Map.of(...))`; cada `@Tool` recebe `ToolContext` como parâmetro) — mecanismo síncrono por chamada, sem depender de estado implícito de thread ou de escopo de request. Ver `tasks-v2.md` §2.
