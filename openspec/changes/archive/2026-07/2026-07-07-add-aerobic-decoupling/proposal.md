...
- **(Fechado — UX, 2026-07-07)** Superfície: persona primária = **coach** → `DetalheTreinoDialog` (aberto pelo botão "Detalhes" do `TreinoCard`). Como nenhum tipo do detalhe carrega os campos calculados do realizado, adiciona-se **`GET /treinos/realizados/{id}` → `TreinoRealizadoOutputDto`** (endpoint novo — único desvio do "sem endpoint novo" original) e o dialog busca o realizado por `treinoRealizadoId` para o badge. Atleta-side (`RecentTrainingsList`) descartado (off-persona).
