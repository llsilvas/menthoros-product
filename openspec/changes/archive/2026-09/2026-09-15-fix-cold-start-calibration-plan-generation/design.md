...
## 14. Modelo de carga cold-start — MOVIDO

O slice de modelo de carga (rampa/cap/piso/banda, redução ×0,5 de RECOVERY/POST_RACE, extensão da alocação ao PROXIMA_SEMANA) foi **separado** para a change `fix-cold-start-load-model` (design próprio + ADR-0012), fechada no grilling de 2026-09-12. Esta change mantém o slice de invariantes/coerência (§13) e baseline (§1–§12), que aguarda as decisões de produto do §13.4.
