...
- [x] 0.1 **Feito.** Usuário do founder com role `ADMIN` no Keycloak de **produção**, via
      `menthoros-infra` (seed/sync).
- [x] 0.2 **Feito.** Vars `SMTP_*` e `FRONTEND_URL` configuradas no serviço `menthoros-backend` do
      Railway (`develop` e `production`).
- [x] 0.3 **Feito.** Domínio verificado no Resend; segundo cliente SMTP com a mesma API key
...
- [x] 4.5 **DECIDIDO: cortado, não implementado.** O founder manteve a sugestão do product review
      de cortar — a validação manual real em `develop` (tasks 5.1–5.2) cobriu o caminho feliz de
      ponta a ponta com backend, Keycloak e e-mail reais, o que o E2E com backend mockado não
      faria. PR mergeado sem este E2E. Sem `tests/e2e/**/convite-fundadora*` no front — apenas
      `convite-atleta.spec.ts`, de outra feature.

## 5. Validação em `develop` e rollout

- [x] 5.1 **Feito.** Em `develop`: convite real testado (e-mail via Resend), aceite, JWT com
      `tenant_id`/`PROPRIETARIO`, `Assessoria GRATUITO 10/1 fundadora=true`, convite
      `converted_at`, `tb_signup_provisioning` com `origin = FOUNDING_INVITE` — confirmados.
- [x] 5.2 **Feito.** Em `develop`: reenvio invalidando o anterior, link expirado e reconsumo após
      conversão validados (404/409 conforme esperado).
- [x] 5.3 **Feito.** `COACH_SIGNUP_ENABLED=false` em `production`; `/cadastro` sem token mostra o
      aviso de convite.
