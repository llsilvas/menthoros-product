...
- [x] 6.5 `src/features/coach/pages/CoachInboxPage.tsx` (novo):
  - **Layout entregue: 3 colunas** (filtros 80px | lista 360px | painel revisão flex-1) — design.md Decisão 6 (2-painéis) substituído pelo layout original do mock (sprint anterior), validado pelo PO.
  - Filtro local: Todas | Pendentes | Aprovadas | Rejeitadas; busca os 3 status em paralelo na montagem.
  - Lista usa `CoachAthleteAvatar` + `SuggestionTypeBadge` + `ConfidenceBar` (mapeando UPPERCASE→lowercase e HIGH/MEDIUM/LOW→número).
  - Painel com tabs Detalhes | Raciocínio da IA; botões Aprovar/Rejeitar desabilitados para status já processados.
  - `useCoachSugestoes` aceita `status?` opcional (retrocompatível com testes existentes).
...
- **Seed de desenvolvimento:** `src/main/resources/db/seed/seed_sugestoes_dev.sql` — script PL/pgSQL para popular `tb_sugestao_coach` localmente (busca o tenant da primeira assessoria ativa e insere sugestões para os 3 primeiros atletas ativos). Não é Flyway — rodar manualmente via `docker run ... psql`.
- **Layout 3-colunas:** `design.md` Decisão 6 (2-painéis) substituída pelo layout 3-colunas do mock da sprint anterior — decisão PO validada durante integração.
