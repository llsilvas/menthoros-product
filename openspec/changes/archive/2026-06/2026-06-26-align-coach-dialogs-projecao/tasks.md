...
- [x] 1.1 Casca → `CoachDialog` (remover papel `#ffffff` + gradiente do title/content).
- [x] 1.2 Boxes de seção (gradientes claros) → `elevation.card` + `content.cardBorder`; chip → tokens.
- [x] 1.3 `DialogActions` (`#f8fafc`/`#e2e8f0`) e botão submit (`#0e3147`) → rodapé `CoachDialog` + botão padrão (`primary[500]`/`surface[900]`).
...
- [x] 2.1 Casca → `CoachDialog`; boxes de tabela/análise/premissas/CTL/goal/metadata → `elevation.card` + `content.cardBorder`.
- [x] 2.2 Tipografia (`#0e3147`/`#4a5568`/`#2d3748`/`#6b7a8d`) → `surface[50]/[200]/[400]/[500]`.
- [x] 2.3 Chip de confiança (mistura hex+token) → `semantic.*` consistente.
  - verify: tabela e gaps legíveis no fundo escuro; fluxo resultado intacto.

## Seção 3 — Objetos de cor + badges

- [x] 3.1 `GAP_ASSESSMENT_COLORS` e `CTL_TREND_COLORS` (hex) → `semantic.success/info/danger` (mapear estado→token).
- [x] 3.2 `ConfidenceBadge` (`CONFIDENCE_STYLES`) → `semantic.danger/warning/success`.
- [x] 3.3 `MarcarOficialButton` (`#27ae60`/`#1e8449`) → `semantic.success[500]/[700]`.
