...
- [x] **1.2** Criar record `TreinoManualInputDto` em `dto/input/`:
...
- [x] **1.2b** Criar método `findFirstForManualMatch` em `TreinoPlanejadoRepository`:
...
- [x] **1.3** Implementar `TreinoService.registrarTreinoManualAtleta(UUID atletaId, TreinoManualInputDto input)`:
...
- [x] **1.4** Criar `AtletaTreinoController` (ou adicionar no controller existente de atleta se houver):
...
- [x] **1.5** Criar endpoint `GET /api/v1/atletas/me/treinos?dias=7` no mesmo controller:
...
- [x] **1.7** Testes: `AtletaTreinoServiceImplTest` — `@Nested` por método:
...
- [x] **1.8** Testes: `AtletaTreinoControllerTest` (`@WebMvcTest`):
...
- [x] **2.1** Criar `ManualTrainingService` em `src/api/services/`:
...
- [x] **2.2** Criar types em `src/types/Treino.ts`:
...
- [x] **2.3** Criar hook `useManualTraining`:
...
- [x] **3.1** Criar `ManualTrainingFormPage` (rota `/atleta/treinos/registrar`):
...
- [x] **3.2** Criar `RecentTrainingsList` component:
...
- [x] **3.3** Adicionar rota `/atleta/treinos/registrar` no `App.tsx` (ou roteador do shell do atleta).
  Adicionar link/botão "Registrar treino de hoje" na `AtletaHomePage` como ação rápida.
  - **verify:** `npm run lint && npm run build`.

- [x] **3.4** Testes: `ManualTrainingForm.test.tsx`:
