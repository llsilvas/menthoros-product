...
**Status:** Concluído — mergeado em `develop` em 2026-06-20
**Trilha:** Full
**Repos:** menthoros-backend + menthoros-front

## Entregue

Todas as 18 tasks implementadas (Seções 1–5). PRs mergeados:
- Backend: `llsilvas/menthoros-backend#6` — 911 testes, 0 falhas
- Frontend: `llsilvas/menthoros-front#4` — 95 testes, 0 falhas

**Ajustes pós-QA (não estavam no scope original):**
- Filtro de data `semanaFim >= hoje` na query de fila de revisão (coach vê apenas planos da semana corrente e futuras)
- Adicionado endpoint `GET /api/v1/coach/planos/revisao?status=` para histórico por status
- `PlanoDetalhePanel` redesenhado para layout minimalista (TreinoTag compactas em vez de SessaoRow com barras)
- `listarPlanosPendentes` refatorado para delegar a `listarPlanosPorStatus` (elimina duplicação)
- `resolveAtletaNome` com guard para `getNome()` nulo
- Toast de sucesso condicionado ao retorno `boolean` de `aprovar`/`rejeitar`

## Deferido (follow-up)

- N+1 queries em `inicializarAssociacoes` → substituir por `JOIN FETCH` em issue separado
- `DiaSemanaDto`/`TreinoPlanejadoDto` duplicados entre `PlanoReview.ts` e `PlanoSemanal.ts` → consolidar
- Lógica de role ATLETA hardcoded em `PlanoTreinoController` → mover para o service
- `@RequireTenant` ausente em `GET /planos/{id}` (pré-existente) → `complete-authorization-controllers`
- Acessibilidade teclado em `PlanoPendenteItem` (`role="button"`, `tabIndex`, `onKeyDown`)
