...
- [x] 0.2 Aplicar por `sync-realm.sh` no HomeLab e no Railway `develop`; conferir **no token emitido** que o composite traz `TECNICO` junto. Nunca pelo console.
  `verify:` **HomeLab ✅ (2026-08-15)** — `sync-realm.sh` aplicado; `PROPRIETARIO` existe com `composite: true` e `composites = ["TECNICO"]`, conferido pela admin API. **Railway `develop` ✅ (2026-08-15)** — `PROPRIETARIO` criada com `composite: true` → `TECNICO`. As credenciais admin são as mesmas do HomeLab (estão no `.env.sync`). **Token real ✅ (2026-08-15)** — JWT emitido pelo `menthoros-test` para o coach de teste traz `realm_access.roles = [PROPRIETARIO, TECNICO, ...]`. A composição funciona no servidor, não só no arquivo.
...
- [x] 0.4 **Decidir e executar o backfill dos coaches existentes — gate de deploy, não de merge.** Rodar a consulta de diagnóstico primeiro e classificar cada assessoria: com um único `TECNICO`, com vários, sem nenhum, com empate de `createdAt`. Só o primeiro caso é automático; os demais viram **lista para atribuição manual**, nunca escolha silenciosa. O backfill tem **duas pernas**: atribuir a role no Keycloak (autoridade) e popular a flag por migration (para quem ainda não logou). **O backend não sobe exigindo `PROPRIETARIO` antes disso** — o coach existente não perde nada que já tinha (nenhum `@PreAuthorize` atual muda), mas ficaria sem acesso ao recurso novo da própria assessoria.
  ...
  **Decisão do founder (1/2):** não marcar dono automaticamente; validar pelo caminho real, criando um coach novo pelo `/cadastro`. Executado — ver 0.4b.
  **Decisão do founder (2/2), 2026-08-15:** marcar o próprio usuário `lsilva.info@gmail.com` como dono do tenant seed, em atribuição manual — o caso `SEM_TECNICO` que a regra automática não resolve. Feito nas duas pernas: role `PROPRIETARIO` atribuída no Keycloak e `owner = true` em `tb_usuario`. Coerência conferida: a Organization `assessoria-demo` do usuário tem `tenant_id = 1b5ce37e-...`, o mesmo `Menthoros Default` do vínculo no banco — então o gate de coerência aprova.
  **Consequência esperada e correta:** a `role` local dele permanece `ADMIN` (a cadeia de `mapToUserRole` prioriza `ADMIN`), então ele **não** aparece em `uso.tecnicos` — o tenant seed segue exibindo zero técnicos. Isso é o comportamento certo: ele é administrador de plataforma, não técnico contratado, e a propriedade vive na flag, não na role.
...
- [x] 3.3 Validar em staging: **role sincronizada e backfill executado antes do deploy do backend** (0.2 e 0.4), métricas de update/upload/falha e feature flag do upload.
  `verify:` ✅ **Railway `develop` sincronizado em 2026-08-15.** `PROPRIETARIO` presente e composite; `menthoros-web` segue com `directAccessGrants: false` e PKCE `S256` (corte do ROPC intacto); `menthoros-test` já estava desligado lá, então o sync não teve o efeito colateral que teve no HomeLab.
  **Armadilha do `sync-realm.sh`, para a próxima pessoa:** ele faz `set -a && . ./.env.sync`, então **variável passada antes do comando é sobrescrita pelo arquivo** — `KEYCLOAK_URL=... ./sync-realm.sh` não muda o alvo. Sincronizar outro ambiente exige carregar o `.env.sync` no shell e chamar o `docker run` com as variáveis já resolvidas, ou manter um arquivo por ambiente.
  **`KC_SMTP_FROM_NAME` precisa ser ajustado por ambiente:** o arquivo local diz `"Menthoros [local]"`, e sincronizar o Railway sem sobrescrever carimbaria os e-mails de dev com o rótulo da máquina do dev. Passado `"Menthoros [dev]"` na execução; conferido depois que o realm manteve o valor correto.
  **Pendente de validação humana:** um coach real abrindo a página no Railway. O backfill de produção não se aplica ainda — não há assessoria de cliente lá.
