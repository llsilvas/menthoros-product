...
**Modelo de carga SEPARADO (grilling 2026-09-12, ADR-0012).** O piloto local com o enforcement ligado expôs que o alvo semanal (`targetTss = ctlAtual×7`) colapsa a 0 no cold-start e não reduz em RECOVERY. Essas decisões (regime cold-start com rampa/cap/piso/banda, ×0,5 de RECOVERY/POST_RACE, extensão da alocação ao `PROXIMA_SEMANA`) foram fechadas e **movidas para a change própria `fix-cold-start-load-model`** (READY, com o ADR-0012). Esta change (`fix-cold-start-calibration-plan-generation`) mantém o slice de **invariantes de etapas/coerência** (§13) e baseline, que ainda depende das decisões de produto do §13.4 (NOT READY).
