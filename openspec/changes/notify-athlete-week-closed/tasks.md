...
- [ ] 1.1 Criar `features/athlete/adapters/selectWeekClosedInfo.ts` — função pura que recebe `PlanoSemanal | null` e retorna `{ semanaEncerrada: boolean; treinosPerdidos: number }`:
  - `semanaEncerrada = plano?.status === 'CONCLUIDO'` (normalizar com `getSafeValue` se `status` vier como object-enum).
...
- [ ] 4.1 `npm run lint && npm run build && npm run test:run` — tudo verde.
- [ ] 4.2 QA (Fast track): `frontend-reviewer` + `clean-code-reviewer` sobre o diff; opcional `/codex:review`.
