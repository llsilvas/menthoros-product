...
- [ ] 1.1 Criar `domain/planner/RacePreparationRule` (`minimoSemanas`, `distanciaNominalKm`,
...
- [ ] 1.3 `ProvaOutputDto` ganha `preparacaoCurta` e `semanasFaltando`; mapper calcula com
      `Clock` injetado.
      *verify:* teste do mapper com data fixa: 8 semanas para maratona → `true`, 20 → `false`,
      exatamente 16 → `false`.
- [ ] 1.4 Prova-alvo única em `criarProva`/`atualizarProva` (desmarca as demais via
...
- [ ] 2.1 Extrair `security/AuthenticatedAtletaResolver` do `resolverAtletaIdAtual` de
...
- [ ] 2.2 `ProvaServiceImpl.resolveAtletaComPosse(atletaId)`: para principal `ATLETA` sem
...
- [ ] 2.3 `ProvaController`: `@PreAuthorize("hasAnyRole('ATLETA','TECNICO','ADMIN')")` em
...
- [ ] 2.4 `ProvaAtletaInputDto` (nome, data, tipo, distância, `distanciaKm`, tempo objetivo,
...
- [ ] 2.5 `cancelarProva` (soft, `statusProva = CANCELADA`) e `ProvaRealizadaImutavelException`
...
- [ ] 3.6 `SugestaoCoachGeneratorJob`: conjunto `MOTIVOS_IGNORADOS = {PROVA_ATLETA}` checado antes
...
- [ ] 4.3 `AthleteRacesPage`: prova-alvo em destaque (semanas faltando × recomendadas, aviso de
...
- [ ] 4.4 `AthleteRaceFormPage`: nome, data (`min` amanhã), chips de distância (+ "Outra" com
...
- [ ] 4.5 `RaceTargetBanner` na `AthletePlanPage` com três estados (alvo / provas sem alvo /
...
- [ ] 4.6 Lint, build e suíte do front verdes.
      *verify:* `npm run lint && npm run build && npm test`.

## 5. Frontend — coach

- [ ] 5.1 `types/Coach.ts` `AttentionReason += 'PROVA_ATLETA'`; `coachInboxHelpers.ts`
...
- [ ] 6.2 Anotar em `add-macrociclo-structure/proposal.md` a dependência da tabela
