...
- [ ] 1.1 Migration aditiva: colunas `next_week_focus` + `focus_outcome` em `tb_revisao_semanal` (tabela da F1) — [D5, D7]
  - verify: `flyway:validate`; colunas presentes.
- [ ] 1.2 Gerar `nextWeekFocus` no encerramento (mesmo hook da F1) via infra LLM, recebendo `recommendationType` como restrição — [CA-LLM]
- [ ] 1.3 Flag `menthoros.weekly-review.llm.enabled`: desligada ⇒ template determinístico derivado do `recommendationType` (fallback desta fatia), sem chamada LLM — [CA-LLM, D5]
- [ ] 1.4 Validação: `./mvnw clean test`

## 2. Insumo na geração do próximo plano

- [ ] 2.1 Injetar a revisão mais recente (`nextWeekFocus` + `recommendationType`) no `PlanoTreinoPromptBuilder`, atrás de flag — [CA4]
...
- [ ] 3.1 Registrar `focusOutcome` (MANTIDO|EDITADO|DESCARTADO) na `tb_revisao_semanal` ao gerar/aprovar o próximo plano — [CA8, D7]
- [ ] 3.2 Expor `focusOutcome` agregável — sinal de aprendizado
...
- [ ] 4.4 Flag desligada → `nextWeekFocus` template determinístico (do `recommendationType`), sem chamada LLM; flag ligada → narrativa consistente com `recommendationType` [CA-LLM]
